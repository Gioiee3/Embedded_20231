...
{  
		...
		lcd_write_command(FUNCTION_SET | OPT_N);				// 4-bit mode 0x28
	    lcd_write_command(CLEAR_DISPLAY);				        // Clear screen
	    lcd_write_command(DISPLAY_ON_OFF_CONTROL | OPT_D);		// Lcd-on, cursor-off, no-blink 0x0C
    	lcd_write_command(ENTRY_MODE_SET | OPT_INC);			// Tu dong dich chuyen con tro den vi tri tiep theo khi xuat ra 1 ki tu
}
/**/
...
/**
 * Write a float variable on the LCD
 */
void Lcd_write_float(float number)
{
	char num[11];
	sprintf(num, "%.2f", number); // @suppress("Float formatting support")
